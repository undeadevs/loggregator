...
def generate_events(n=1, min_dup_split=0.2, *, topic: str|None = None, event_id: uuid.UUID|None = None, timestamp: datetime.datetime|None = None, source: str|None = None, payload: dict|None = None):
    dup_split = uniform(min_dup_split, 0.5)
    ...
    for i in range(n):
        ...
        if is_dup and dup_n > 0 and len(events) > 0:
            ...
            unq_n -= 1
            events.append(Event(
                topic=randstr(5,10) if topic is None else topic, 
